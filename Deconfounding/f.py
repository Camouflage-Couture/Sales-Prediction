...
def train_dualvae(model, weights, dataset, device, use_scm=False, num_epochs=100, batch_size=128, learning_rate=1e-3, run_id=1):
    ...
    for epoch in range(num_epochs):
        ...
        wandb.log({
            f"DualVAE_Training_Progress_SCM_{use_scm}_Epoch": epoch + 1,
            f"DualVAE_Training_Progress_SCM_{use_scm}_Total_Loss": avg_loss,
            f"DualVAE_Training_Progress_SCM_{use_scm}_Reconstruction_X_Loss": loss_dict["recon_X_loss"].item() / len(dataloader.dataset),
            f"DualVAE_Training_Progress_SCM_{use_scm}_Reconstruction_Y_Loss": loss_dict["recon_Y_loss"].item() / len(dataloader.dataset),
            f"DualVAE_Training_Progress_SCM_{use_scm}_KL_Divergence": loss_dict["kld_total"].item() / len(dataloader.dataset),
            f"DualVAE_Training_Progress_SCM_{use_scm}_Orthogonality_ZX_S": loss_dict["ortho_loss_zx_s"].item() / len(dataloader.dataset),
            f"DualVAE_Training_Progress_SCM_{use_scm}_Orthogonality_ZY_S": loss_dict["ortho_loss_zy_s"].item() / len(dataloader.dataset),
            f"DualVAE_Training_Progress_SCM_{use_scm}_Conditional_Independence_Loss": (loss_dict["ci_loss"].item() / len(dataloader.dataset)) if use_scm else 0.0,
            f"DualVAE_Training_Progress_SCM_{use_scm}_Inference_ZX_Loss": loss_dict["inference_zx_loss"].item() / len(dataloader.dataset)
        })
    ...
    plt.legend()
    plt.tight_layout()
    plot_path = f"DualVAE_Training_Run_{run_id}_SCM_{use_scm}.png"
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    wandb.log({f"DualVAE_Training_Progress_SCM_{use_scm}": wandb.Image(plot_path)})
...
def plot_performance_comparison(rewards_no_scm, rewards_scm, run_id):
    plt.figure(figsize=(10,5))
    sns.set(style="whitegrid")
    plt.plot(rewards_no_scm, label='No_SCM', color='red')
    plt.plot(rewards_scm, label='With_SCM', color='blue')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
...
def run_experiment(run_id, config, confounder_level, confounder_amplitudes, use_scm):
    ...
    wandb.log({"Causal_Index": causal_index})
    ...
    for episode in tqdm(range(1, max_episodes_causal + 1), desc=f"Run {run_id}: PPO Training ({'With SCM' if use_scm else 'No SCM'})", leave=False):
        ...
        if episode % 100 == 0:
            ...
            if use_scm:
                wandb.log({"PPO_With_SCM_Average_Reward_Last_100": avg_reward})
            else:
                wandb.log({"PPO_No_SCM_Average_Reward_Last_100": avg_reward})
            print(f"Run {run_id}: Episode {episode}, Average Reward (Last 100): {avg_reward:.2f}")

    # Phase 5: Logging and Plotting
    run_data = {
        f"PPO_{'With_SCM' if use_scm else 'No_SCM'}_Rewards": rewards_causal
    }
    wandb.log(run_data)
    ...
    wandb.log({
        f"PPO_{'With_SCM' if use_scm else 'No_SCM'}_Average_Reward_ATE": avg_reward_causal
    })
...
def main():
    ...
    wandb.log({
        "Performance_With_SCM_Mean_Reward": mean_reward_scm,
        "Performance_With_SCM_Std_Reward": std_reward_scm,
        "Performance_No_SCM_Mean_Reward": mean_reward_no_scm,
        "Performance_No_SCM_Std_Reward": std_reward_no_scm
    })

    # Plot aggregated performance
    plt.figure(figsize=(10,5))
    sns.set(style="whitegrid")
    plt.boxplot([rewards_runs_no_scm.flatten(), rewards_runs_with_scm.flatten()], labels=['No_SCM', 'With_SCM'])
    plt.ylabel('Reward')
    ...
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    wandb.log({"Final_Performance_Comparison": wandb.Image(plot_path)})
    ...
    plt.figure(figsize=(10,5))
    sns.set(style="whitegrid")
    plt.plot(mean_rewards_no_scm, label='No_SCM', color='red')
    plt.plot(mean_rewards_scm, label='With_SCM', color='blue')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    ...
    plt.close()
    wandb.log({"Average_PPO_Performance": wandb.Image(plot_path_avg)})
