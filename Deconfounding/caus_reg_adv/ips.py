...
class CausalPPOAgent:
    ...
    def update_policy(self, memory):
        ...
        # Log losses to wandb
        wandb.log({
            "Causal PPO/Policy Loss": loss.item(),
            "Causal PPO/Entropy": entropy.mean().item()
        })

        print(f"Causal PPO Update - Policy Loss: {loss.item():.4f}, Entropy: {entropy.mean().item():.4f}")

# Frontdoor Actor-Critic Model for PPO
class PPOActorCriticFrontdoor(nn.Module):
    """
    Actor-Critic network for PPO with Frontdoor Adjustment.
    """
    def __init__(self, state_dim, mediator_dim, action_dim):
...
class FrontdoorPPOAgent:
    ...
    """
    def __init__(self, state_dim, mediator_dim, action_dim, lr=3e-4, gamma=0.99, K_epochs=10, eps_clip=0.2):
        self.policy = PPOActorCriticFrontdoor(state_dim, mediator_dim, action_dim).to(device)
        self.optimizer = optim.Adam(self.policy.parameters(), lr=lr)
        self.policy_old = PPOActorCriticFrontdoor(state_dim, mediator_dim, action_dim).to(device)
        self.policy_old.load_state_dict(self.policy.state_dict())
        self.gamma = gamma
        self.eps_clip = eps_clip
        self.K_epochs = K_epochs
    ...
    def update_policy(self, memory):
        ...
        # Log losses to wandb
        wandb.log({
            "Frontdoor PPO/Policy Loss": loss.item(),
            "Frontdoor PPO/Entropy": entropy.mean().item()
        })

        print(f"Frontdoor PPO Update - Policy Loss: {loss.item():.4f}, Entropy: {entropy.mean().item():.4f}")

# --------------------------- Inverse Propensity Scoring (IPS) PPO Agent Definitions --------------------------- #

class IPSPPOActorCritic(nn.Module):
    """
    Actor-Critic model for IPS PPO Agent.
    """
    def __init__(self, state_dim, action_dim):
        super(IPSPPOActorCritic, self).__init__()
        self.actor = nn.Sequential(
            nn.Linear(state_dim, 256),
            nn.ReLU(),
            nn.Linear(256, action_dim),
...
class IPSPPOAgent:
    ...
    def __init__(self, state_dim, action_dim, treatment_dim, lr=3e-4, gamma=0.99, K_epochs=10, eps_clip=0.2):
        ...
        # Actor-Critic model
        self.policy = IPSPPOActorCritic(state_dim, action_dim).to(device)
        self.optimizer = optim.Adam(self.policy.parameters(), lr=lr)
        self.policy_old = IPSPPOActorCritic(state_dim, action_dim).to(device)
        self.policy_old.load_state_dict(self.policy.state_dict())
        self.gamma = gamma
        self.eps_clip = eps_clip
        self.K_epochs = K_epochs
...
# --------------------------- Main Training Loop with Baseline, Causal, Frontdoor, and IPS PPO Agents --------------------------- #
...
def main():
    ...
    for confounding_level in wandb.config.confounding_levels:
        ...
        ppo_agent_frontdoor = FrontdoorPPOAgent(state_dim, mediator_dim=4, action_dim=action_dim,
                                                lr=wandb.config.ppo_lr,
                                                gamma=wandb.config.ppo_gamma,
                                                K_epochs=wandb.config.ppo_K_epochs,
                                                eps_clip=wandb.config.ppo_eps_clip)  # Frontdoor PPO
        ...
        ppo_agent_ips = IPSPPOAgent(state_dim=state_dim,
                                    ...
                                    gamma=wandb.config.ppo_gamma,
                                    K_epochs=wandb.config.ppo_K_epochs,
                                    eps_clip=wandb.config.ppo_eps_clip)  # IPS PPO
        ...
        # --------------------------- Training Frontdoor PPO Agent --------------------------- #

        print("Starting training of Frontdoor PPO agent with Frontdoor Adjustment...")
        ...
        for episode in tqdm(range(1, max_episodes + 1), desc=f"IPS PPO Training ({confounding_level})"):
            state = env.reset()
            ep_reward = 0
            for t in range(max_timesteps):
                timestep += 1
            ...
            if episode % 50 == 0 or episode == 1:
                ...
                wandb.log({f"IPS PPO/Average Reward ({confounding_level})": avg_reward, "Episode": episode})
    ...
    for confounding_level in wandb.config.confounding_levels:
        ...
        rewards_ips = results[confounding_level]['rewards_ips']
        ...
        plt.plot(rewards_ips, label=f'IPS PPO ({confounding_level.capitalize()})')
        plt.xlabel('Episode')
        plt.ylabel('Total Reward')
        plt.title(f'PPO Agents Performance Comparison at {confounding_level.capitalize()} Confounding Level')
        plt.legend()
        plt.grid(True)
        # Save the plot to a file
        ...
        window_size = 100
        if len(rewards_baseline) >= window_size and len(rewards_causal) >= window_size and len(rewards_frontdoor) >= window_size and len(rewards_ips) >= window_size:
            ...
            plt.title(f'PPO Agents Performance Moving Average at {confounding_level.capitalize()} Confounding Level')
            plt.legend()
            plt.grid(True)
            # Save the plot to a file
            ...
            print(f"Not enough episodes to compute moving average for {confounding_level} confounding level.")
