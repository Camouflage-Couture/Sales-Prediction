...
def main():
    ...
    ppo_agent_causal = FrontdoorPPOAgent(state_dim, latent_dim=4, action_dim=action_dim,
                                         lr=wandb.config.ppo_lr,
                                         gamma=wandb.config.ppo_gamma,
                                         K_epochs=wandb.config.ppo_K_epochs,
    ...
    memory_causal = {'states': [], 'mediators': [], 'actions_prev': [], 'actions': [], 'logprobs': [], 'rewards': [], 'is_terminals': []}
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Causal PPO Training"):
        ...
        for t in range(max_timesteps):
            ...
            action, log_prob = ppo_agent_causal.select_action(state, mediator, action_prev)
            next_state, reward, done, _ = env.step(action)
            # Save data in memory
