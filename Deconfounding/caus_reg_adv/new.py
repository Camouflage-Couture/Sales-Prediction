...
def dualvae_loss(x, y, action, output, weights, num_actions=4):
    ...
    if action is not None:
        # Simple prediction of previous action from mediators
        action_prev = action.long()
        action_pred_sX = nn.Linear(output.sample["s_X"].size(1), num_actions).to(device)
        action_pred_sY = nn.Linear(output.sample["s_Y"].size(1), num_actions).to(device)
        pred_action_sX = action_pred_sX(output.sample["s_X"])
        pred_action_sY = action_pred_sY(output.sample["s_Y"])
        equiv_loss_sX = nn.functional.cross_entropy(pred_action_sX, action_prev)
        equiv_loss_sY = nn.functional.cross_entropy(pred_action_sY, action_prev)
...
class BaselinePPOAgent:
    ...
    def update_policy(self, memory):
        ...
        # Log losses to wandb
        wandb.log({
            "Baseline PPO/Policy Loss": loss.item(),
            "Baseline PPO/Entropy": entropy.mean().item()
        })

        print(f"Baseline PPO Update - Policy Loss: {loss.item():.4f}, Entropy: {entropy.mean().item():.4f}")

# Front-Door Actor-Critic Model for PPO
...
class FrontdoorPPOAgent:
    ...
    def update_policy(self, memory):
        ...
        # Log losses to wandb
        wandb.log({
            "Front-Door PPO/Policy Loss": loss.item(),
            "Front-Door PPO/Entropy": entropy.mean().item()
        })

        print(f"Front-Door PPO Update - Policy Loss: {loss.item():.4f}, Entropy: {entropy.mean().item():.4f}")

# Inverse Propensity Scoring (IPS) PPO Agent

class IPSPPOAgent:
    """
    PPO Agent that incorporates Inverse Propensity Scoring to adjust for confounders.
    """
    def __init__(self, state_dim, action_dim, treatment_dim, lr=3e-4, gamma=0.99, K_epochs=10, eps_clip=0.2):
        ...
        # Treatment Model for propensity scores
        self.treatment_model = nn.Sequential(
            nn.Linear(treatment_dim, 64),
            nn.ReLU(),
            nn.Linear(64, action_dim)
        ).to(device)
        self.optimizer_treatment = optim.Adam(self.treatment_model.parameters(), lr=lr)

    def select_action(self, state, treatment):
        """
        Select action using the current policy.
        """
        state = torch.FloatTensor(state).unsqueeze(0).to(device)
        treatment = torch.FloatTensor(treatment).unsqueeze(0).to(device)
        with torch.no_grad():
            action, log_prob, _ = self.policy_old.act(state)
        return action, log_prob.item()

    def estimate_propensity_scores(self, treatments, actions):
        """
        Estimate propensity scores P(A | Z) using the treatment model.
        """
        self.treatment_model.train()
        optimizer = self.optimizer_treatment
        optimizer.zero_grad()
        logits = self.treatment_model(treatments)
        loss = nn.CrossEntropyLoss()(logits, actions)
        loss.backward()
        optimizer.step()
        return

    def compute_propensity_scores(self, treatments):
        """
        Compute propensity scores P(A | Z) using the trained treatment model.
        """
    ...
    def update_policy(self, memory):
        """
        Update the PPO policy using the stored memory and IPS adjustment.
        """
        old_states = torch.FloatTensor(memory['states']).to(device)
        old_actions = torch.LongTensor(memory['actions']).to(device)
        old_logprobs = torch.FloatTensor(memory['logprobs']).to(device)
        rewards = memory['rewards']
        is_terminals = memory['is_terminals']
        treatments = torch.FloatTensor(memory['treatments']).to(device)

        # Estimate propensity scores
        self.estimate_propensity_scores(treatments, old_actions)
        propensity_scores = self.compute_propensity_scores(treatments)
        propensity_scores = propensity_scores.gather(1, old_actions.unsqueeze(1)).squeeze(1)  # P(A|Z)

        # Adjust rewards using IPS
        adjusted_rewards = rewards / (propensity_scores.cpu().numpy() + 1e-6)  # Avoid division by zero
        ...
        # Log losses to wandb
        wandb.log({
            "IPS PPO/Policy Loss": loss.item(),
            "IPS PPO/Entropy": entropy.mean().item()
        })

        print(f"IPS PPO Update - Policy Loss: {loss.item():.4f}, Entropy: {entropy.mean().item():.4f}")

# --------------------------- Main Training Loop with Baseline, Frontdoor, and IPS PPO Agents --------------------------- #

def main():
    # Initialize wandb project
    wandb.init(project="DualVAE_PPO_Experiment", config={
        "num_data_steps": 10000,  # Reduced from 100000
        "dualvae_epochs": 10,     # Reduced from 100
        "dualvae_batch_size": 64, # Reduced from 128
        "dualvae_learning_rate": 1e-3,
        "ppo_max_episodes": 1000, # Reduced from 20000
        "ppo_max_timesteps": 200,
        "ppo_update_timestep": 4000,  # Can be adjusted based on max_episodes and max_timesteps
        "ppo_lr": 3e-4,               # Reduced from 3e-4
        "ppo_gamma": 0.99,
        "ppo_K_epochs": 10,            # Reduced from 10
        "ppo_eps_clip": 0.2,
        "dualvae_loss_weights": [1.0, 1.0, 0.1, 0.1, 0.1, 0.001, 0.001],
        "confounding_level": "medium",
        # Additional configs for IPS PPO Agent
        "ips_lr": 3e-4,
        "ips_gamma": 0.99,
        "ips_K_epochs": 10,
        "ips_eps_clip": 0.2,
    ...
    ppo_agent_frontdoor = FrontdoorPPOAgent(state_dim=state_dim,
                                           mediator_dim=4,
                                           confounder_dim=4,
                                           ...
                                           gamma=wandb.config.ppo_gamma,
                                           K_epochs=wandb.config.ppo_K_epochs,
                                           eps_clip=wandb.config.ppo_eps_clip)  # Front-Door PPO

    ppo_agent_ips = IPSPPOAgent(state_dim=state_dim,
                                action_dim=action_dim,
                                treatment_dim=4,  # Assuming shared latents are 4-dimensional
                                lr=wandb.config.ips_lr,
                                gamma=wandb.config.ips_gamma,
                                K_epochs=wandb.config.ips_K_epochs,
                                eps_clip=wandb.config.ips_eps_clip)  # IPS PPO
    ...
    # Initialize memories for PPO agents
    memory_baseline = {'states': [], 'actions': [], 'logprobs': [], 'rewards': [], 'is_terminals': []}
    memory_frontdoor = {'states': [], 'mediators': [], 'confounders': [], 'actions_prev': [], 'actions': [], 'logprobs': [], 'rewards': [], 'is_terminals': []}
    memory_ips = {'states': [], 'actions': [], 'logprobs': [], 'rewards': [], 'is_terminals': [], 'treatments': []}
    ...
    # --------------------------- Training Front-Door PPO Agent --------------------------- #

    print("Starting training of Front-Door PPO agent with Frontdoor Adjustment...")
    for episode in tqdm(range(1, max_episodes + 1), desc="Front-Door PPO Training"):
        state = env.reset()
        ...
        for t in range(max_timesteps):
            ...
            # Select action using Front-Door PPO agent
            action, log_prob = ppo_agent_frontdoor.select_action(state, mediator, confounder, action_prev)
            next_state, reward, done, _ = env.step(action)
            # Save data in memory
            memory_frontdoor['states'].append(state)
            memory_frontdoor['mediators'].append(mediator)
            memory_frontdoor['confounders'].append(confounder)
        ...
        if episode % 50 == 0 or episode == 1:
            ...
            wandb.log({"Front-Door PPO/Average Reward": avg_reward, "Episode": episode})
            print(f"Front-Door PPO Agent - Episode {episode}, Average Reward (last 50): {avg_reward:.2f}")
    print("\nTraining of Front-Door PPO agent completed.\n")

    # --------------------------- Training IPS PPO Agent --------------------------- #

    print("Starting training of IPS PPO agent with Inverse Propensity Scoring...")
    for episode in tqdm(range(1, max_episodes + 1), desc="IPS PPO Training"):
        state = env.reset()
        ep_reward = 0
        for t in range(max_timesteps):
            timestep += 1
            # Obtain treatment representation from DualVAE
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(device)
            dualvae.eval()
            with torch.no_grad():
                output = dualvae(state_tensor, state_tensor)
                treatment = output.sample["s_X"].cpu().numpy()[0]  # 4-dimensional treatment

            # Select action using IPS PPO agent
            action, log_prob = ppo_agent_ips.select_action(state, treatment)
            next_state, reward, done, _ = env.step(action)
            # Save data in memory
            memory_ips['states'].append(state)
            memory_ips['actions'].append(action)
            memory_ips['logprobs'].append(log_prob)
            memory_ips['rewards'].append(reward)
            memory_ips['is_terminals'].append(done)
            memory_ips['treatments'].append(treatment)
            state = next_state
            ep_reward += reward
            # Update PPO agent
            if timestep % update_timestep == 0:
                ppo_agent_ips.update_policy(memory_ips)
                memory_ips = {'states': [], 'actions': [], 'logprobs': [], 'rewards': [], 'is_terminals': [], 'treatments': []}
            if done:
                break
        rewards_ips.append(ep_reward)
        # Logging every 50 episodes
        if episode % 50 == 0 or episode == 1:
            avg_reward = np.mean(rewards_ips[-50:])
            wandb.log({"IPS PPO/Average Reward": avg_reward, "Episode": episode})
            print(f"IPS PPO Agent - Episode {episode}, Average Reward (last 50): {avg_reward:.2f}")
    print("\nTraining of IPS PPO agent completed.\n")
    ...
    plt.plot(rewards_frontdoor, label='Front-Door PPO')
    plt.plot(rewards_ips, label='IPS PPO')
    plt.xlabel('Episode')
    plt.ylabel('Total Reward')
    plt.title('PPO Agents Performance Comparison')
    plt.legend()
    plt.grid(True)
    # Save the plot to a file
    ...
    window_size = 100
    if all(len(rewards) >= window_size for rewards in [rewards_baseline, rewards_frontdoor, rewards_ips]):
        ...
        moving_avg_ips = np.convolve(rewards_ips, np.ones(window_size)/window_size, mode='valid')

        plt.figure(figsize=(12, 6))
        plt.plot(moving_avg_baseline, label=f'Baseline PPO (Moving Avg {window_size})')
        plt.plot(moving_avg_frontdoor, label=f'Front-Door PPO (Moving Avg {window_size})')
        plt.plot(moving_avg_ips, label=f'IPS PPO (Moving Avg {window_size})')
        plt.xlabel('Episode')
        plt.ylabel('Moving Average Total Reward')
        plt.title('PPO Agents Performance Comparison with Moving Average')
        plt.legend()
        plt.grid(True)
        # Save the plot to a file
        plot_ma_filename = "ppo_moving_average_comparison.png"
        plt.savefig(plot_ma_filename)
        plt.close()
        # Log the plot to wandb
        wandb.log({"PPO/Moving Average Comparison": wandb.Image(plot_ma_filename)})
    else:
