...
class ConfounderEnvWrapper(gym.Wrapper):
    ...
    def reset(self):
        ...
        confounder = sum(
            amp * np.sin(0) + amp * np.cos(0)
            for amp in self.confounder_amplitudes
        )
        modified_state = state + confounder + np.random.normal(0, self.noise_std, size=state.shape)
...
class VAEOutput:
    ...
    def set_latent_zero(self, zero_vars):
        ...
        assert isinstance(zero_vars, list), "zero_vars should be a list of keys to zero out."
        for key in zero_vars:
            if key in self.sample:
                self.sample[key] = torch.zeros_like(self.sample[key])
...
class CausalPPOAgent:
    ...
    def __init__(self, state_dim, latent_dim, action_dim, causal_var_indices, lr=3e-4, gamma=0.99, K_epochs=10, eps_clip=0.2, causal_weight=0.1):
        ...
        self.dualvae = None  # Will be set later

    def set_dualvae(self, dualvae_model):
        """
        Set the DualVAE model for the agent.
        """
        self.dualvae = dualvae_model
    ...
    def update_policy(self, memory, causal_index):
        ...
        for _ in range(self.K_epochs):
            ...
            reg_loss = 0.0
            for idx in range(len(self.policy.causal_var_indices)):
                # Only penalize weights corresponding to non-causal variables
                if idx not in self.policy.causal_var_indices:
                    # Access the state_encoder's first Linear layer weights
                    reg_loss += torch.norm(self.policy.state_encoder[0].weight[:, idx], 2)
        ...
        if causal_index > threshold_high:
            # High causal signal: decrease regularization to allow more flexibility
            ...
            # Low causal signal: increase regularization to enforce focus on causal features
            self.causal_weight *= (1 + adjustment_factor)
        ...
        print(f"PPO Update - Policy Loss: {policy_loss_avg:.4f}, Entropy: {entropy_avg:.4f}, Regularization: {regularization_avg:.4f}, Causal Weight: {self.causal_weight:.4f}")
...
# --------------------------- Causal Curiosity PPO Agent Definitions --------------------------- #

class CausalCuriosityPPOAgent:
    """
    PPO Agent that leverages the causal index to guide policy learning and exploration.
    """
    def __init__(self, state_dim, latent_dim, action_dim, base_lr=3e-4, gamma=0.99, K_epochs=10, eps_clip=0.2,
                 lambda_causal=0.1, beta_lr=0.5):
        self.policy = PPOActorCritic(state_dim, latent_dim, action_dim, causal_var_indices=[]).to(device)
        ...
        self.eps_clip = eps_clip
        self.K_epochs = K_epochs
        self.lambda_causal = lambda_causal  # Regularization strength
        self.beta_lr = beta_lr  # Learning rate modulation factor
        self.mse_loss = nn.MSELoss()
        self.dualvae = None  # Will be set later
        self.causal_index = None  # Will be updated during training
    ...
    def set_causal_index(self, causal_index):
        """
        Set the causal index for the agent.
        """
    ...
    def select_action(self, state):
        ...
        with torch.no_grad():
            # Obtain shared latents from DualVAE
            output = self.dualvae(state_tensor, state_tensor)  # Assuming y = x for simplicity
            s_x = output.sample["s_x"].cpu().numpy()[0]  # 4-dimensional

            # Normalize the shared latents to compute feature importance weights
            feature_weights = np.abs(s_x) / (np.sum(np.abs(s_x)) + 1e-8)

            # Store feature weights for later use in policy update
            self.feature_weights = feature_weights

            # Use policy to select action
            action, log_prob, _ = self.policy_old.act(state_tensor, torch.FloatTensor(s_x).unsqueeze(0).to(device))
        return action, log_prob.item(), s_x

    def update_policy(self, memory):
        """
        Update the PPO policy using the stored memory and causal regularization.
        """
        old_states = torch.FloatTensor(memory['states']).to(device)
        old_latents = torch.FloatTensor(memory['latents']).to(device)
        old_actions = torch.LongTensor(memory['actions']).to(device)
        ...
        state_values = self.policy.critic(torch.cat([self.policy.state_encoder(old_states), self.policy.latent_encoder(old_latents)], dim=-1)).squeeze(-1)
        advantages = returns - state_values.detach()
        advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-7)

        # Adjust learning rate based on causal index
        if self.causal_index is not None:
            adjusted_lr = self.base_lr * (1 + self.beta_lr * self.causal_index)
        ...
        for _ in range(self.K_epochs):
            ...
            reg_loss = 0.0
            for idx in range(len(self.feature_weights)):
                # Penalize policy's reliance on less important features
                reg_loss += self.feature_weights[idx] * torch.norm(self.policy.state_encoder[0].weight[:, idx], 2)
            loss += self.lambda_causal * reg_loss
            regularization_total += (self.lambda_causal * reg_loss).item()
        ...
        # Log losses to wandb
        wandb.log({
            "Causal Curiosity PPO/Policy Loss": policy_loss_avg,
            "Causal Curiosity PPO/Entropy": entropy_avg,
            "Causal Curiosity PPO/Regularization": regularization_avg,
            "Causal Curiosity PPO/Adjusted LR": adjusted_lr
        })

        print(f"Causal Curiosity PPO Update - Policy Loss: {policy_loss_avg:.4f}, Entropy: {entropy_avg:.4f}, Regularization: {regularization_avg:.4f}, Adjusted LR: {adjusted_lr:.6f}")
...
def main():
    ...
    ppo_agent_causal.set_dualvae(dualvae)

    # Initialize Causal Curiosity PPO agent
    ppo_agent_causal_curiosity = CausalCuriosityPPOAgent(state_dim, latent_dim=4, action_dim=action_dim,
                                                         base_lr=wandb.config.ppo_lr,
                                                         gamma=wandb.config.ppo_gamma,
                                                         K_epochs=wandb.config.ppo_K_epochs,
                                                         eps_clip=wandb.config.ppo_eps_clip,
                                                         lambda_causal=0.1,
                                                         beta_lr=0.5)
    ppo_agent_causal_curiosity.set_dualvae(dualvae)
    ...
    memory_causal_curiosity = {'states': [], 'latents': [], 'actions': [], 'logprobs': [], 'rewards': [], 'is_terminals': []}

    # Initialize rewards tracking
    rewards_baseline = []
    rewards_causal = []
    rewards_causal_curiosity = []
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Causal PPO Training"):
        ...
        for t in range(max_timesteps):
            ...
            if timestep % update_timestep == 0:
                ...
                causal_index = compute_causal_index(dualvae, dataloader)
                ppo_agent_causal.set_causal_index(causal_index)
                ppo_agent_causal.update_policy(memory_causal, causal_index=causal_index)
                # Log causal index
                wandb.log({"Causal PPO/Causal Index": causal_index, "Episode": episode})
                memory_causal = {'states': [], 'latents': [], 'actions': [], 'logprobs': [], 'rewards': [], 'is_terminals': []}
            if done:
                break
    ...
    # --------------------------- Training Causal Curiosity PPO Agent --------------------------- #

    print("Starting training of Causal Curiosity PPO agent...")
    for episode in tqdm(range(1, max_episodes + 1), desc="Causal Curiosity PPO Training"):
        state = env.reset()
        ep_reward = 0
        for t in range(max_timesteps):
            timestep += 1
            # Select action using Causal Curiosity PPO agent
            action, log_prob, s_x = ppo_agent_causal_curiosity.select_action(state)
            next_state, reward, done, _ = env.step(action)
            # Save data in memory
            memory_causal_curiosity['states'].append(state)
            memory_causal_curiosity['latents'].append(s_x)
            memory_causal_curiosity['actions'].append(action)
            memory_causal_curiosity['logprobs'].append(log_prob)
            memory_causal_curiosity['rewards'].append(reward)
            memory_causal_curiosity['is_terminals'].append(done)
            state = next_state
            ep_reward += reward
            # Update PPO agent
            if timestep % update_timestep == 0:
                # Compute causal index
                causal_index = compute_causal_index(dualvae, dataloader)
                ppo_agent_causal_curiosity.set_causal_index(causal_index)
                ppo_agent_causal_curiosity.update_policy(memory_causal_curiosity)
                # Log causal index
                wandb.log({"Causal Curiosity PPO/Causal Index": causal_index, "Episode": episode})
                memory_causal_curiosity = {'states': [], 'latents': [], 'actions': [], 'logprobs': [], 'rewards': [], 'is_terminals': []}
            if done:
                break
        rewards_causal_curiosity.append(ep_reward)
        # Logging every 50 episodes
        if episode % 50 == 0 or episode == 1:
            avg_reward = np.mean(rewards_causal_curiosity[-50:])
            wandb.log({"Causal Curiosity PPO/Average Reward": avg_reward, "Episode": episode})
            print(f"Causal Curiosity PPO Agent - Episode {episode}, Average Reward (last 50): {avg_reward:.2f}")
    print("\nTraining of Causal Curiosity PPO agent completed.\n")
    ...
    plt.plot(rewards_causal, label='Causal PPO')
    plt.plot(rewards_causal_curiosity, label='Causal Curiosity PPO')
    plt.xlabel('Episode')
    ...
    window_size = 100
    if len(rewards_baseline) >= window_size and len(rewards_causal) >= window_size and len(rewards_causal_curiosity) >= window_size:
        ...
        moving_avg_causal_curiosity = np.convolve(rewards_causal_curiosity, np.ones(window_size)/window_size, mode='valid')
        ...
        plt.plot(moving_avg_causal_curiosity, label=f'Causal Curiosity PPO (Moving Avg {window_size})')
        plt.xlabel('Episode')
