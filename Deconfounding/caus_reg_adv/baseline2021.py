...
class BaselinePPOAgent:
    ...
    def update_policy(self, memory):
        ...
        # Compute advantages
        # Here, state_values are estimated by the critic
        with torch.no_grad():
...
def main():
    ...
    training_params = {
        ...
        "ppo_max_episodes": 10000,       # RL agent training epochs
    ...
    )# The BaselinePPOAgent already moves policies to device
    ...
    for episode in range(1, training_params["ppo_max_episodes"] + 1):
        ...
        for t in range(training_params["ppo_max_timesteps"]):
            ...
            if timestep % training_params["ppo_update_timestep"] == 0:
                ppo_agent.update_policy(memory)
                # Optionally, log the loss to wandb
                # wandb.log({"PPO/Loss": loss})

        # Logging
        reward_history.append(episode_reward)
        if len(reward_history) > moving_average_window:
    ...
    # Final PPO Update if memory has remaining steps
    ...
    plt.title(f'PPO Agent Training Rewards (Average over Last {moving_average_window} Episodes)')
    plt.grid(True)
    plt.show()
