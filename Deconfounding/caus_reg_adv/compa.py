...
# Causal PPO Agent without Frontdoor Adjustment
class PPOActorCriticCausal(nn.Module):
    """
    Actor-Critic network for PPO with causal regularization (without frontdoor adjustment).
    """
    def __init__(self, state_dim, latent_dim, action_dim):
        super(PPOActorCriticCausal, self).__init__()
...
class CausalPPOAgent:
    """
    PPO Agent with causal regularization (without frontdoor adjustment).
    """
    def __init__(self, state_dim, latent_dim, action_dim, lr=3e-4, gamma=0.99, K_epochs=10, eps_clip=0.2):
        self.policy = PPOActorCriticCausal(state_dim, latent_dim, action_dim).to(device)
        self.optimizer = optim.Adam(self.policy.parameters(), lr=lr)
        self.policy_old = PPOActorCriticCausal(state_dim, latent_dim, action_dim).to(device)
        self.policy_old.load_state_dict(self.policy.state_dict())
        self.gamma = gamma
        self.eps_clip = eps_clip
        self.K_epochs = K_epochs
...
def main():
    ...
    wandb.init(project="Frontdoor_PPO_Comparison", config={
    ...
    ppo_agent_causal = CausalPPOAgent(state_dim, latent_dim=4, action_dim=action_dim,
                                      ...
                                      gamma=wandb.config.ppo_gamma,
                                      K_epochs=wandb.config.ppo_K_epochs,
                                      eps_clip=wandb.config.ppo_eps_clip)  # Causal PPO without Frontdoor
    ppo_agent_frontdoor = FrontdoorPPOAgent(state_dim, latent_dim=4, action_dim=action_dim,
                                            lr=wandb.config.ppo_lr,
                                            gamma=wandb.config.ppo_gamma,
                                            K_epochs=wandb.config.ppo_K_epochs,
                                            eps_clip=wandb.config.ppo_eps_clip)  # Causal PPO with Frontdoor
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Baseline PPO Training"):
        ...
        for t in range(max_timesteps):
            ...
            ep_reward += reward
            # Update PPO agent
            if timestep_baseline % update_timestep == 0:
        ...
        if episode % 500 == 0 or episode == 1:
            avg_reward = np.mean(rewards_baseline[-500:])
            wandb.log({"Baseline PPO/Average Reward": avg_reward, "Episode": episode})
            print(f"Baseline PPO Agent - Episode {episode}, Average Reward (last 500): {avg_reward:.2f}")
    print("\nTraining of Baseline PPO agent completed.\n")

    # --------------------------- Training Causal PPO Agent without Frontdoor Adjustment --------------------------- #

    print("Starting training of Causal PPO agent without Frontdoor Adjustment...")
    for episode in tqdm(range(1, max_episodes + 1), desc="Causal PPO Training"):
        state = env.reset()
        ep_reward = 0
        for t in range(max_timesteps):
            timestep_causal += 1
            # Obtain latent representation from DualVAE (shared latents)
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(device)
            dualvae.eval()
            with torch.no_grad():
                output = dualvae(state_tensor, state_tensor)  # Assuming y = x for simplicity
                latent = output.sample["s_x"].cpu().numpy()[0]  # 4-dimensional latent
            ...
            ep_reward += reward
            # Update PPO agent
            if timestep_causal % update_timestep == 0:
        ...
        # Logging every 500 episodes
        if episode % 500 == 0 or episode == 1:
            avg_reward = np.mean(rewards_causal[-500:])
            wandb.log({"Causal PPO/Average Reward": avg_reward, "Episode": episode})
            print(f"Causal PPO Agent - Episode {episode}, Average Reward (last 500): {avg_reward:.2f}")
    print("\nTraining of Causal PPO agent completed.\n")

    # --------------------------- Training Causal-enhanced PPO Agent with Frontdoor Adjustment --------------------------- #

    print("Starting training of Causal-enhanced PPO agent with Frontdoor Adjustment...")
    for episode in tqdm(range(1, max_episodes + 1), desc="Causal Frontdoor PPO Training"):
        state = env.reset()
        action_prev = 0  # Initialize previous action
        ep_reward = 0
        for t in range(max_timesteps):
            timestep_frontdoor += 1
            ...
            ep_reward += reward
            # Update PPO agent
            if timestep_frontdoor % update_timestep == 0:
        ...
        # Logging every 500 episodes
        if episode % 500 == 0 or episode == 1:
            avg_reward = np.mean(rewards_frontdoor[-500:])
            wandb.log({"Causal Frontdoor PPO/Average Reward": avg_reward, "Episode": episode})
            print(f"Causal Frontdoor PPO Agent - Episode {episode}, Average Reward (last 500): {avg_reward:.2f}")
    print("\nTraining of Causal Frontdoor PPO agent completed.\n")
    ...
    plt.plot(rewards_causal, label='Causal PPO without Frontdoor')
    plt.plot(rewards_frontdoor, label='Causal PPO with Frontdoor')
    plt.xlabel('Episode')
    ...
    if len(rewards_baseline) >= window_size and len(rewards_causal) >= window_size and len(rewards_frontdoor) >= window_size:
        ...
        plt.plot(moving_avg_frontdoor, label=f'Causal Frontdoor PPO (Moving Avg {window_size})')
        plt.xlabel('Episode')
